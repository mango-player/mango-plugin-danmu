...
var danmakuAPI = '//galaxy.bz.mgtv.com';
...
var chimeeDanmu = {
    ...
    init: function init(videoConfig) {
        ...
        (0, _mangoHelper.addEvent)(document, 'visibilitychange', this.onVisibilityChange);
    ...
    destroy: function destroy() {
        ...
        (0, _mangoHelper.removeEvent)(document, 'visibilitychange', this.onVisibilityChange);
    },
    events: {
        ...
        seek: function seek() {
            ...
            this.cache.forEach(function (list) {
        ...
        timeupdate: function timeupdate() {
            ...
            if (this.datamode == 'batch') {
                ...
                var index = Math.floor(this._currentTime / this.interval);
                if (!this.cache[index]) {
                ...
                if (this._currentTime - this.lastCurrentTime > 2000 && this.cache[index]) {
                    ...
                    var find = this.cache[index].filter(function (item) {
                        if (item.time - _this._currentTime < 200 && !item._sent) {
                            return true;
                        } else {
                            return false;
                        }
        ...
        // CMS返回结果 请求前贴片广告
        cmsDataComplete: function cmsDataComplete(data) {
    ...
    methods: {
        _getConfig: function _getConfig() {
            var _this = this;
            var url = danmakuAPI + '/getctlbarrage?' + params({
            ...
            return (0, _mangoHelper.fetchJsonp)(url).then(function (response) {
        ...
        _fetchDanmu: function _fetchDanmu(time) {
            var _this = this;
            var url = danmakuAPI + '/rdbarrage?' + params({
                version: this.version,
                vid: this.videoinfo.video_id,
                abroad: 0,
                pid: this.videoinfo.playlist_id,
            ...
            return (0, _mangoHelper.fetchJsonp)(url, { timeout: 10 * 1000 }).then(function (response) {
                return response.json();
            }).then(function (json) {
                if (json && json.status == 0 && json.data) {
                    _this.interval = json.data.interval * 1000;
                    var time_1 = json.data.next - _this.interval / 2;
                    // 计算当前是第几段弹幕 ，如果存在弹幕数据，则进行缓存
                    var index = Math.floor(time_1 / _this.interval);
                    _this.cache[index] = json.data.items || [];
                }
            });
        },
...
var Danma = /** @class */function () {
    ...
    Danma.prototype.emit = function (data) {
        ...
        this.thread.pool.push({
            ...
            text: data.text,
            mode: data.mode,
            speed: Math.pow(piece.width, 1 / 3) * 0.2,
            row: row,
            offset: {
